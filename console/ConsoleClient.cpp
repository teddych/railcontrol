...
namespace console
{
	...
	void ConsoleClient::HandleStreetNew(string& s, size_t& i)
	{
		...
		if (manager.StreetSave(StreetNone, name, datamodel::Street::DefaultDelay, datamodel::Street::CommuterTypeBoth, relations, VisibleNo, 0, 0, 0, AutomodeNo, fromTrack, fromDirection, toTrack, toDirection, FeedbackNone, FeedbackNone, feedbackID, FeedbackNone, result) == false)
		{
			SendAndPrompt(result);
			return;
		}
