...
namespace console
{
	...
	void ConsoleClient::HandleTrackNew(string& s, size_t& i)
	{
		...
		string result;
		if (manager.TrackSave(TrackNone, name, posX, posY, posZ, length, rotation, datamodel::Track::TrackTypeStraight, feedbacks, datamodel::Track::SelectStreetSystemDefault, false, result) > TrackNone)
		{
			SendAndPrompt(result);
			return;
		}
	...
	void ConsoleClient::HandleLocoNew(string& s, size_t& i)
	{
		...
		string result;
		if (!manager.LocoSave(LocoNone, name, control, protocol, address, functions, 0, false, MaxSpeed, DefaultTravelSpeed, DefaultReducedSpeed, DefaultCreepSpeed, result))
		{
			SendAndPrompt(result);
			return;
		}
	...
	void ConsoleClient::HandleStreetNew(string& s, size_t& i)
	{
		...
		if (manager.StreetSave(StreetNone, name, datamodel::Street::DefaultDelay, datamodel::Street::CommuterTypeBoth, 0, 0, relations, VisibleNo, 0, 0, 0, AutomodeNo, fromTrack, fromDirection, toTrack, toDirection, FeedbackNone, FeedbackNone, feedbackID, FeedbackNone, 0, result) == false)
		{
			SendAndPrompt(result);
			return;
		}
